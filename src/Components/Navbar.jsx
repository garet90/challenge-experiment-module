...
const Navbar = () => {
  ...
  const [noWalletIsOpen, setNoWalletIsOpen] = useState(false);
  const location = useLocation();

  const { connectMetamaskWithAccount, account } = useContext(MainContext);

  const connectMetamask = useCallback(() => {
    connectMetamaskWithAccount().then((present) => {
      if (!present) {
        setNoWalletIsOpen(true);
      }
    })
  }, [setNoWalletIsOpen, connectMetamaskWithAccount]);

  const toggleMenu = useCallback(() => {
    setMenuIsOpen(!menuIsOpen);
  }, [setMenuIsOpen, menuIsOpen]);

  const dismissNoWallet = useCallback(() => {
    setNoWalletIsOpen(false);
  }, [setNoWalletIsOpen]);

  return (
    <>
      {noWalletIsOpen ? (
        <NoWalletModal dismiss={dismissNoWallet} />
      ) : (<></>)}
          ...
          <div className="flex justify-between items-center w-full">
            ...
            <div className="flex justify-center items-center gap-2 md:hidden">
              ...
              <button
                ...
                onClick={toggleMenu}
              >
                {menuIsOpen ? <LiaTimesSolid /> : <HiOutlineMenuAlt3 />}
              </button>
            </div>
            {/* Mobile Menu Button */}
          </div>
